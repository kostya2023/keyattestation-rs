...
use crate::{
    asn1::{
        oids::{KEY_DESCRIPTION_OID, PROVISIONING_INFO_MAP_OID},
        schemas::{
            KeyDescription as KeyDescriptionASN1, ProvisioningInfoMap as ProvisioningInfoMapASN1,
        },
    },
    domain::schemas::{
        KeyDescription as KeyDescriptionDomain, ProvisioningInfoMap as ProvisioningInfoMapDomain,
    },
};
use rasn::der;
...
impl<'attest> Attestation<'attest> {
    pub fn new(chain: CertificateChain<'attest>) -> Result<Self, KeyAttestationError> {
        ...
        let key_description = extensions
            ...
            .ok_or_else(|| {
                KeyAttestationError::AttestationError(
                    "KeyDescription extension not found.".to_string(),
                )
            })
            .and_then(|ext| {
                let asn: KeyDescriptionASN1 = der::decode(ext.value).map_err(|_| {
                    KeyAttestationError::AttestationError(
                        "KeyDescription parsing error.".to_string(),
                    )
                })?;
                asn.try_into()
            })?;
    ...
    pub fn verify(
        ...
        if self.key_description.attestation_challenge != challenge {
            return Err(KeyAttestationError::AttestationError(
                "Attestation verify error.".to_string(),
            ));
        }
