...
impl ConstraintsVerifier for ConstraintVerifierMatchesCertificate {
    fn verify(&self, attestation: &Attestation) -> Result<(), KeyAttestationError> {
        ...
        let security_level = security_level
            .ok_or_else(|| KeyAttestationError::ConstraintsVerifyError("Constraints verify error.".to_string()))?;
        ...
        let security_level = match security_level {
            ...
            _ => Err(KeyAttestationError::ConstraintsVerifyError("Constraints verify error.".to_string())),
        }?;

        if attestation.key_description.attestation_security_level != security_level || attestation.key_description.key_mint_security_level != security_level {
